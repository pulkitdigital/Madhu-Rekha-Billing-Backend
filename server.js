...
app.get("/api/bills/:id", async (req, res) => {
  ...
  try {
    ...
    // Oldest first in history (already sorted)
    const primaryPayment = payments[0] || null;
    ...
    res.json({
      ...
      total,
      paid: netPaid, // net paid after refunds
...
app.get("/api/bills/:id/invoice-html-pdf", async (req, res) => {
  ...
  try {
    ...
    // 3) Load payments to get primary payment details and totals
    const paysSnap = await db
      .collection("payments")
      .where("billId", "==", id)
      .get();
    ...
    // 4) Load refunds to compute net paid
    const refundsSnap = await db
      .collection("refunds")
      .where("billId", "==", id)
      .get();
    ...
    doc
      .rect(boxX, boxY, boxWidth, lineH * 5 + 4)
      .stroke();
...
app.get("/api/refunds/:id/refund-html-pdf", async (req, res) => {
  ...
  try {
    ...
    // 4) Load all refunds for this bill to compute cumulative upto THIS refund
    const refundsSnap = await db
      .collection("refunds")
      .where("billId", "==", billId)
      .get();
...
// Returns today / this month / this year totals for payments and refunds
app.get("/api/dashboard/summary", async (_req, res) => {
  try {
    ...
    // TODAY (use == for better index usage)
