...
app.get("/api/bills/:id/full-payment-pdf", async (req, res) => {
  ...
  try {
    ...
    doc
      .font(fontOr("WS-Bold"))
      .fontSize(14)
      .text("MADHUREKHA EYE CARE CENTRE", 0, y + 6, {
        align: "center",
        width: pageWidth,
      });
    doc
    ...
    // const lineH2 = 14;
    // const rows2 = 6;
    // const boxH = lineH2 * rows2 + 8;
    // if (y + boxH > doc.page.height - 120) { doc.addPage(); y = 36; }
    // doc.rect(boxX, y, boxW, boxH).stroke();

    // let by = y + 6;
    // doc.font(fontOr("WS")).fontSize(9);

    // const computedSubtotal = Number(bill.subtotal || items.reduce((s, it) => s + Number(it.amount || 0), 0)).toFixed(2);
    ...
    doc
      .fontSize(8)
      .text("Patient / Representative", marginLeft, sigY + 4, {
        width: sigWidth,
        align: "center",
      });
    const rightSigX = pageWidth - 36 - sigWidth;
