...
//     if (firstPay > 0) {
//   // OLD: const receiptNo = await generateReceiptId(invoiceNo);
//   // FIXED: pass billId
//   const receiptNo = await generateReceiptId(invoiceNo, billId);
...
app.get("/api/bills/:id/summary-pdf", async (req, res) => {
  ...
  try {
    ...
    doc
      .fontSize(8)
      .text("For Madhurekha Eye Care Centre", rightSigX, sigY2 + 4, {
        width: rightSigWidth,
        align: "center",
      });
...
// Full Payment PDF: /api/bills/:id/full-payment-pdf
...
app.get("/api/bills/:id/full-payment-pdf", async (req, res) => {
  ...
  try {
    ...
    // fetch items (use services if present else legacy items)
    let items = [];
    ...
    const payments = paysSnap.docs
      .map((doc) => {
        const d = doc.data();
        const paymentDateTime =
          d.paymentDateTime || (d.paymentDate ? `${d.paymentDate}T00:00:00.000Z` : null);
        return {
          id: doc.id,
          amount: Number(d.amount || 0),
          paymentDate: d.paymentDate || null,
          paymentTime: d.paymentTime || null,
          paymentDateTime,
          receiptNo: d.receiptNo || null,
          mode: d.mode || "",
          referenceNo: d.referenceNo || null,
    ...
    // refunds (if needed for totals)
    const refundsSnap = await db
      .collection("refunds")
      .where("billId", "==", billId)
      .get();
    const refunds = refundsSnap.docs.map((doc) => Number((doc.data() && doc.data().amount) || 0));

    // totals
    const billTotal = Number(bill.total || 0);
    const totalPaidGross = payments.reduce((s, p) => s + Number(p.amount || 0), 0);
    const totalRefunded = refunds.reduce((s, r) => s + Number(r || 0), 0);
    const netPaid = totalPaidGross - totalRefunded;
    const balance = billTotal - netPaid;

    // small helpers (local to this function so it's self-contained)
    function formatMoney(v) {
      return Number(v || 0).toFixed(2);
    }
    function formatDateTime(dtString) {
    ...
    // PDF headers / resources (kept same style as your other PDFs)
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `inline; filename="full-payment-${billId}.pdf"`);
    ...
    // header logos & title (identical style)
    try { doc.image(logoLeftPath, 36, y, { width: 45, height: 45 }); } catch (e) {}
    try { doc.image(logoRightPath, pageWidth - 36 - 45, y, { width: 45, height: 45 }); } catch (e) {}

    doc.font("Helvetica-Bold").fontSize(16).text("MADHUREKHA EYE CARE CENTRE", 0, y + 4, { align: "center", width: pageWidth });
    doc.font("Helvetica").fontSize(9)
      .text("SONARI: E-501, Sonari East Layout, Near Sabuz Sangh Kali Puja Maidan, Jamshedpur - 831011", 0, y + 24, { align: "center", width: pageWidth })
      .text("PAN : ABFFM3115J   |   Reg. No: 2035700023", { align: "center", width: pageWidth });

    y += 60;
    doc.moveTo(36, y).lineTo(pageWidth - 36, y).stroke();
    y += 6;

    // static doctors (unchanged)
    doc.fontSize(9).font("Helvetica-Bold");
    doc.text("Dr. Pradipta Kundu", 36, y);
    doc.text("Dr. (Mrs.) Amita Kundu", pageWidth / 2, y, { align: "right", width: usableWidth / 2 });
    y += 12;
    doc.font("Helvetica").fontSize(8);
    doc.text("Reg. No.: 28873", 36, y);
    doc.text("Reg. No.: 16219", pageWidth / 2, y, { align: "right", width: usableWidth / 2 });
    y += 18;

    // Title of this PDF
    doc.rect(36, y, usableWidth, 20).stroke();
    doc.font("Helvetica-Bold").fontSize(12).text("FULL PAYMENT RECEIPT (DETAILED)", 36, y + 4, { align: "center", width: usableWidth });
    y += 26;

    // Patient info block
    doc.font("Helvetica-Bold").fontSize(9).text("Patient Details:", 36, y);
    y += 12;
    doc.font("Helvetica").fontSize(9);
    doc.text(`Name: ${bill.patientName || ""}`, 36, y, { width: usableWidth * 0.6 });
    doc.text(`Invoice No.: ${bill.invoiceNo || billId}`, pageWidth / 2, y, { align: "right", width: usableWidth / 2 });
    y += 12;
    doc.text(`Age: ${bill.age != null ? bill.age + " Years" : ""}`, 36, y, { width: usableWidth * 0.6 });
    doc.text(`Date: ${bill.date || ""}`, pageWidth / 2, y, { align: "right", width: usableWidth / 2 });
    y += 12;
    doc.text(`Sex: ${bill.sex || ""}`, 36, y, { width: usableWidth * 0.6 });
    doc.text(`Balance: Rs ${formatMoney(balance)}`, pageWidth / 2, y, { align: "right", width: usableWidth / 2 });
    y += 12;
    doc.text(`Address: ${bill.address || ""}`, 36, y, { width: usableWidth });
    y += 18;

    // Services / Treatment table header
    doc.font("Helvetica-Bold").fontSize(9);
    doc.text("Treatments / Services", 36, y);
    y += 10;
    ...
    const colRateW = 90;
    const colAmtW = 90;
    const colDescW = usableWidth - (colQtyW + colRateW + colAmtW);
    const colDescX = svcLeft;
    const colQtyX = colDescX + colDescW;
    const colRateX = colQtyX + colQtyW;
    const colAmtX = colRateX + colRateW;

    // header row
    doc.save().rect(svcLeft, y, usableWidth, 16).fill("#F3F3F3").restore().rect(svcLeft, y, usableWidth, 16).stroke();
    doc.font("Helvetica-Bold").fontSize(8);
    doc.text("Description", colDescX + 2, y + 3, { width: colDescW - 4 });
    doc.text("Qty", colQtyX + 2, y + 3, { width: colQtyW - 4, align: "right" });
    doc.text("Rate", colRateX + 2, y + 3, { width: colRateW - 4, align: "right" });
    doc.text("Amount", colAmtX + 2, y + 3, { width: colAmtW - 4, align: "right" });
    y += 16;
    doc.font("Helvetica").fontSize(9);

    items.forEach((it, idx) => {
      const rowH = 14;
      doc.rect(svcLeft, y, usableWidth, rowH).stroke();
      doc.text(it.description || "", colDescX + 2, y + 3, { width: colDescW - 4 });
      doc.text(it.qty != null ? String(it.qty) : "", colQtyX + 2, y + 3, { width: colQtyW - 4, align: "right" });
      doc.text(formatMoney(it.rate || 0), colRateX + 2, y + 3, { width: colRateW - 4, align: "right" });
      doc.text(formatMoney(it.amount || 0), colAmtX + 2, y + 3, { width: colAmtW - 4, align: "right" });
      y += rowH;
    ...
    doc.font("Helvetica-Bold").fontSize(9).text("Payment Details (chronological):", 36, y);
    y += 10;

    // Table columns: Date Time | ReceiptNo | Mode | Reference | DrawnOn/DrawnAs | Amount
    const tLeft = 36;
    const colDateW2 = 110;
    const colRecW = 90;
    const colModeW2 = 80;
    const colRefW2 = 100;
    const colDrawW = 120;
    const colAmtW2 = usableWidth - (colDateW2 + colRecW + colModeW2 + colRefW2 + colDrawW);
    const cDateX = tLeft;
    const cRecX = cDateX + colDateW2;
    const cModeX = cRecX + colRecW;
    const cRefX = cModeX + colModeW2;
    const cDrawX = cRefX + colRefW2;
    const cAmtX = cDrawX + colDrawW;

    // header row
    doc.save().rect(tLeft, y, usableWidth, 16).fill("#F3F3F3").restore().rect(tLeft, y, usableWidth, 16).stroke();
    doc.font("Helvetica-Bold").fontSize(8);
    doc.text("Date & Time", cDateX + 2, y + 3, { width: colDateW2 - 4 });
    doc.text("Receipt No.", cRecX + 2, y + 3, { width: colRecW - 4 });
    doc.text("Mode", cModeX + 2, y + 3, { width: colModeW2 - 4 });
    doc.text("Reference", cRefX + 2, y + 3, { width: colRefW2 - 4 });
    doc.text("Drawn On / As", cDrawX + 2, y + 3, { width: colDrawW - 4 });
    doc.text("Amount (Rs)", cAmtX + 2, y + 3, { width: colAmtW2 - 4, align: "right" });
    ...
    payments.forEach((p) => {
      const rowH = 14;
      // draw row box
      doc.rect(tLeft, y, usableWidth, rowH).stroke();

      const dt = p.paymentDateTime ? formatDateTime(p.paymentDateTime) : (p.paymentDate ? `${p.paymentDate} ${p.paymentTime || ""}` : "");
      doc.text(dt, cDateX + 2, y + 3, { width: colDateW2 - 4 });
      doc.text(p.receiptNo || "", cRecX + 2, y + 3, { width: colRecW - 4 });
      doc.text(p.mode || "", cModeX + 2, y + 3, { width: colModeW2 - 4 });
      doc.text(p.referenceNo || "", cRefX + 2, y + 3, { width: colRefW2 - 4 });
      const drawSummary = (p.drawnOn ? `On: ${p.drawnOn}` : "") + (p.drawnAs ? (p.drawnOn ? " / " : "") + `As: ${p.drawnAs}` : "");
      doc.text(drawSummary || "-", cDrawX + 2, y + 3, { width: colDrawW - 4 });
      doc.text(formatMoney(p.amount || 0), cAmtX + 2, y + 3, { width: colAmtW2 - 4, align: "right" });
    ...
    const lineH = 14;
    doc.rect(boxX, y, boxW, lineH * 6 + 8).stroke();
    doc.font("Helvetica").fontSize(9);
    let by = y + 6;
    const addRow = (label, value) => {
    ...
    addRow("Net Paid", `Rs ${formatMoney(netPaid)}`);
    addRow("Balance", `Rs ${formatMoney(balance)}`);
    addRow("Payments Count", String(payments.length));

    y = by + 18;

    // Footer note + signatures (same style)
    ...
    doc.fontSize(8).text("Patient / Representative", 36, sigY + 4, { width: sigWidth, align: "center" });
    const rightSigX = pageWidth - 36 - sigWidth;
    doc.moveTo(rightSigX, sigY).lineTo(rightSigX + sigWidth, sigY).dash(1, { space: 2 }).stroke().undash();
    doc.fontSize(8).text("For Madhurekha Eye Care Centre", rightSigX, sigY + 4, { width: sigWidth, align: "center" });
