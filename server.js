...
function invalidateBill(billId) {
  cache.del(makeCacheKey("bill-detail", billId));
  cache.del(makeCacheKey("bills", "list"));
  cache.del(makeCacheKey("transactions", "all"));
  cache.del(makeCacheKey("dashboard", "summary"));
}

function invalidateProfile() {
  cache.del(makeCacheKey("profile", "clinic"));
}
  ...
  try {
    ...
    if (paymentsSnap) {
      paymentsSnap.forEach((doc) => {
        const p = doc.data();
        if (!isInRange(p.paymentDate)) return;
    ...
    if (refundsSnap) {
      refundsSnap.forEach((doc) => {
        const r = doc.data();
        if (!isInRange(r.refundDate)) return;
    ...
    const dateRangeText =
      from && to
        ? `Period: ${formatDateDot(from)} to ${formatDateDot(to)}`
        : from
          ? `From: ${formatDateDot(from)}`
          : to
            ? `To: ${formatDateDot(to)}`
            : "All Dates";
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${type || "all"}-transactions-${new Date().toISOString().slice(0, 10)}.pdf"`,
    );
    ...
    doc
      .fontSize(9)
      .font("Helvetica")
      .text(dateRangeText, 36, y, { align: "center", width: usableWidth });
    y += 14;
    doc.text(
      `Generated on: ${formatDateDot(new Date().toISOString())}`,
      36,
      y,
    ...
    // ─── Summary Box ──────────────────────────────────────────
    const boxHeight = 60;
    doc.rect(36, y, usableWidth, boxHeight).stroke();
    doc
      .font("Helvetica-Bold")
      .fontSize(10)
      .text("Summary", 42, y + 6);
    ...
    doc.text("Total Payments:", 42, y);
    doc.text(`Rs ${totalPayments.toFixed(2)}`, 42, y + 12);
    doc.text("Total Refunds:", 220, y);
    doc.text(`Rs ${totalRefunds.toFixed(2)}`, 220, y + 12);
    doc.text("Net Amount:", 400, y);
    doc.text(`Rs ${netAmount.toFixed(2)}`, 400, y + 12);
    y += 40;
    ...
    const colAmtW =
      usableWidth -
      (colSrW + colDateW + colTypeW + colBillW + colPatientW + colModeW);

    const colSrX = tableLeft;
    const colDateX = colSrX + colSrW;
    ...
    function drawTableHeader(yPos) {
      doc
        .save()
        .rect(tableLeft, yPos, usableWidth, headerHeight)
        .fill("#F3F3F3")
        .restore()
        .rect(tableLeft, yPos, usableWidth, headerHeight)
        .stroke();
      ...
      doc.text("Sr.", colSrX + 4, yPos + 4, { width: colSrW - 8 });
      doc.text("Date", colDateX + 4, yPos + 4, { width: colDateW - 8 });
      doc.text("Type", colTypeX + 4, yPos + 4, { width: colTypeW - 8 });
      doc.text("Bill #", colBillX + 4, yPos + 4, { width: colBillW - 8 });
      doc.text("Patient", colPatientX + 4, yPos + 4, {
        width: colPatientW - 8,
      });
      doc.text("Mode", colModeX + 4, yPos + 4, { width: colModeW - 8 });
      doc.text("Amount", colAmtX + 4, yPos + 4, {
        width: colAmtW - 8,
        align: "right",
      });
    }
    ...
    // ─── Table Rows ───────────────────────────────────────────
    const tableStartY = y; // track where table body starts (for vertical lines on first page)
    let currentPageTableStartY = tableStartY;
    ...
    for (let idx = 0; idx < transactions.length; idx++) {
      ...
      if (y + minRowHeight > doc.page.height - 80) {
        // Draw vertical lines for current page before adding new page
        [
          colSrX,
          colDateX,
          colTypeX,
          ...
          doc
            .moveTo(x, currentPageTableStartY - headerHeight)
            .lineTo(x, y)
            .stroke();
        });
        ...
        y += headerHeight;
        doc.moveTo(tableLeft, y).lineTo(tableRightX, y).stroke();
        currentPageTableStartY = y;
        doc.font("Helvetica").fontSize(8);
      }

      doc.text(`${idx + 1}`, colSrX + 4, y + 3, { width: colSrW - 8 });
      doc.text(formatDateDot(txn.date), colDateX + 4, y + 3, {
        width: colDateW - 8,
      });
      doc.text(txn.type, colTypeX + 4, y + 3, { width: colTypeW - 8 });
      doc.text(`#${txn.invoiceNo}`, colBillX + 4, y + 3, {
        width: colBillW - 8,
      });
      doc.text(txn.patientName, colPatientX + 4, y + 3, {
        width: colPatientW - 8,
      });
      doc.text(txn.mode, colModeX + 4, y + 3, { width: colModeW - 8 });
      doc.text(
        `${txn.type === "Payment" ? "+" : "-"} Rs ${txn.amount.toFixed(2)}`,
        colAmtX + 4,
        y + 3,
...
app.get("/api/payments/:id/receipt-html-pdf", async (req, res) => {
  ...
  try {
    ...
    // Draw Sign.png above dotted line
    const signImagePath = path.join(__dirname, "resources", "Sign.png");
    const signImageW = 100;
    const signImageH = 40;
    const signImageX = rightSigX + (sigWidth - signImageW) / 2;

    try {
      if (fs.existsSync(signImagePath)) {
        doc.image(signImagePath, signImageX, sigY, {
          width: signImageW,
          height: signImageH,
        });
      }
    ...
    // Dotted line below signature image
    doc
      .moveTo(rightSigX, sigY + signImageH + 4)
      .lineTo(rightSigX + sigWidth, sigY + signImageH + 4)
      .dash(1, { space: 2 })
      .stroke()
      .undash();

    // Clinic representative text below dotted line
    doc
      .fontSize(8)
      .font("Helvetica")
      .text(
        clinicRepresentative || "For Madhurekha Eye Care Centre",
        rightSigX,
        sigY + signImageH + 10,
        {
          width: sigWidth,
...
app.get("/api/bills/:id/full-payment-pdf", async (req, res) => {
  ...
  try {
    ...
    const signImageW = 100;
    const signImageH = 40;
    const signImageX = rightSigX + (160 - signImageW) / 2;

    try {
      if (fs.existsSync(signImagePath)) {
        doc.image(signImagePath, signImageX, y, {
          width: signImageW,
          height: signImageH,
        });
      }
    ...
    // Dotted line BELOW the signature image
    doc
      .moveTo(rightSigX, y + signImageH + 4)
      .lineTo(rightSigX + 160, y + signImageH + 4)
      .dash(1, { space: 2 })
      .stroke()
      .undash();
